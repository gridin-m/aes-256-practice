# S-Box из спецификации FIPS 197
S_BOX = [
...
# преобразует 16 байт в матрицу состояния 4x4 по столбцам
def bytes_to_state(data):
    if len(data) != 16:
        raise ValueError(f"Ожидалось 16 байт, получено {len(data)}")
    state = []
    for i in range(4):
        row = []
        for j in range(4):
...
# преобразует матрицу состояния 4x4 обратно в 16 байт
